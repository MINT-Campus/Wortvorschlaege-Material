    ...
    def on_check_clicked(b):
        if feedback_messages is None:
            with output_widget:
                ...
                if not any(selected):
                    output.printError(translations[wl]["nothing_selected"])
                    return
                ...
                if not wrong_selected and not missing_correct:
                    output.printSuccess(translations[wl]["success"])
                elif wrong_selected:
                    output.printError(translations[wl]["wrong"])
                else:
                    output.printError(translations[wl]["missing"])
        else:
            with output_widget:
                output_widget.clear_output()
                ...
                selected = [cb.value for cb, _ in checkboxes]
                if not any(selected):
                    output.printError(feedback_messages["nothing_selected"])
                    return
                ...
                if not wrong_selected and not missing_correct:
                    output.printSuccess(feedback_messages["success"])
                elif wrong_selected:
                    output.printError(feedback_messages["wrong"])
                else:
                    output.printError(feedback_messages["missing"])
